...
#define DATA_ACQUISITION_TIME 500 //perform action every 1000ms
#define DATA_MAX_LEN 1200         //bytes
//#define RESET_ESP_STORAGE   //Warning: Do not uncomment this line
#define SETUP_LED 32
#define RTC_LED 14
...
void setup()
{
    ...
    if (can.init_can())
    { //true if initialize success and TODO: battery ids read
        digitalWrite(CAN_LED, HIGH);
        flags[can_f] = 1;
    }
    else
    { //sound alarm and do nothing!
        while (1)
        {
            flags[can_f] = 0;
            delay(100);
    ...
    }
    {
        ...
        device_id = new char[30];
        bt.bluetooth_name.toCharArray(device_id, strlen(bt.bluetooth_name.c_str()) + 1);
        if (bt.bluetooth_name == "" && bt.bluetooth_password == "")
        { //settings saved previously
            settings__.end();
            log_i("Settings found!\r\nbluetooth name: %s\r\n bluetooth password: %s", bt.bluetooth_name.c_str(), bt.bluetooth_password.c_str());
        }
        else
    ...
    device_id = "EV111";
    bt.bluetooth_name = "EV111";
    bt.bluetooth_password = "123456";
    if (bt.init())
    {
        flags[bt_f] = 1;
    }
    ...
    String data = "05:04:41,EV111,5.019,0.234,3000,34.36,01,BATT1,30,80,50,22,11.06,20.561,02,BATT3,30,80,50,22,11.06,20";

    while (1)
    {
        data = data + '/';
        Serial.println(getTime2());
        storage.write_data(getTime2(), data);
        String read_test = storage.read_data();
        storage.mark_data(getTime2());
        Serial.println(read_test);
        vTaskDelay(10000);
    }

    //    xTaskCreatePinnedToCore(vBlCheck, "Bluetooth Commands", 5000, NULL, 2, &blTask1, 0);
    //    xTaskCreatePinnedToCore(vBlTransfer, "Bluetooth Transfer", 4000, NULL, 3, &blTask2, 0);
    //    xTaskCreatePinnedToCore(vWifiTransfer, "Transfer data on Wifi", 5000, NULL, 1, &wifiTask, 0);
    //    xTaskCreatePinnedToCore(vStatusLed, "Status LED", 5000, NULL, 1, &ledTask, 1);
    //    xTaskCreatePinnedToCore(vAcquireData, "Data Acquisition", 5000, NULL, 3, &dataTask1, 1);
    //    xTaskCreatePinnedToCore(vStorage, "Storage Handler", 5000, NULL, 2, &storageTask, 1);
...
void vAcquireData(void *pvParameters)
{
    ...
    {                                               //infinite loop
        ...
        log_i("getting data from CAN");
        {
            ...
            {
                log_i("currently sending data %d", dataflag);
                addSlotsData("01", "BATT1", "30", "80", "50", "22", String(randvoltage), "20.561");
                towrite += ",";
                addSlotsData("02", "BATT3", "30", "80", "50", "22", String(randvoltage), "20.561");
                towrite += ",";
                addSlotsData("03", "BATT5", "30", "80", "50", "22", String(randvoltage), "20.561");
                towrite += ",";
                addSlotsData("04", "BATT7", "30", "80", "50", "22", String(randvoltage), "26.561"); //towrite += ",";
            }
            else if (dataflag == 1)
            ...
            {
                log_i("currently sending data %d", dataflag);
                addSlotsData("01", "BATT2", "BSS22", "30", "50", "22", String(randvoltage), "20.561");
                towrite += ",";
                addSlotsData("02", "BATT4", "BSS22", "30", "50", "22", String(randvoltage), "20.561");
                towrite += ",";
                addSlotsData("03", "BATT6", "BSS22", "30", "50", "22", String(randvoltage), "20.561");
                towrite += ",";
                addSlotsData("04", "BATT8", "BSS22", "30", "50", "22", String(randvoltage), "26.561"); //towrite += ",";
            }
            else if (dataflag >= 3)
...
void vStorage(void *pvParameters)
{
    ...
    { //infinite loop
        ...
        {                                                //storage is working
            ...
            xSemaphoreGive(semaWifi1); //resume the wifi transfer task    //signal to call bluetooth transfer function once
            // xSemaphoreGive(semaStorage1);
            UBaseType_t uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
            log_v("Stack usage of Storage Task: %d", (int)uxHighWaterMark);
        }
